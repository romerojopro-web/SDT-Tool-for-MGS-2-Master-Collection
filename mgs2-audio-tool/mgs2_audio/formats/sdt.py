...
def looks_like_psadpcm(stream: bytes, sample_units: int = 3000) -> bool:
    ...
    In valid PS-ADPCM every 16-byte frame starts with a header byte whose high
    nibble (the predictor filter) is 0-4, and a flag byte that is small (0-7).
    Other codecs (e.g. XWMA/WMA found in some vanilla files) fail this: their
    "filter" nibble spreads across 0-15 and the flag byte takes arbitrary values.
    Returns False when too many frames look invalid.
    """
    if len(stream) < 32:
        return False
    bad = 0
    ...
    for off in range(0, min(len(stream), sample_units * 16), 16):
        header = stream[off]
        flag = stream[off + 1]
        if (header >> 4) & 0xF > 4 or flag > 7:
            bad += 1
        tot += 1
    return tot > 0 and (bad / tot) < 0.1
